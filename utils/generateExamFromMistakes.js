...
import getUserPastExamMistakes from "./getUserPastExamMistakes.js";
...
const genExamFromMistakes = async (userId) => {
  try {
    const mistakes = await getUserPastExamMistakes(userId);

    if (!mistakes) {
      throw new Error("User past mistakes not found");
    }

    const SYSTEM_INSTRUCTION = `
    You are an API designed to assist Vietnamese students in practicing for their ${examSchemaConfig.description}.
    Based on the user's most recent performance, focus on the following mistakes:
      - Common mistakes: ${JSON.stringify(mistakes)}
    The total number of questions must be 50. Part 1, 2, 3, 4, 5 and 12, each part must have 2 questions. Part 6 must have 15 questions. Part 7 and 8, each part must have 5 questions. Part 9 must have 7 questions and 2 passages. Part 10 and 11, each part must have 3 questions. Part 2 questions must have list of options containing only one word, do not include the ' symbol (apostrophe). The options for each question must start with A., B., C. or D. The questions for each part should include only the question or sentence itself, without the question number. The suggestions and detailed answers for each question must be in Vietnamese. The suggestion for each question should be short. Additionally, you need to return the percentage match between the real exam and the generated one after all exam parts have been provided, number only, without the percent sign. You return JSON formatted according to this JSON schema:
    ...
    for (let i = 1; i <= 12; i++) {
      const partSchema =
        examSchemaConfig.properties.parts.properties[`part${i}`];
    ...
    const percentagePart = [{ text: "Give me the percentage value." }];

    const percentageResult = await model.generateContent({
      contents: [{ role: "user", parts: percentagePart }],
...
export default genExamFromMistakes;
